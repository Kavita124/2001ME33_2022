...
def octant_analysis(mod=5000):
	...
	def process_file(s,mod):										# Function to process individual file
		...
		lst=['T','a','b','c','a Avg','b Avg','c Avg',r"a'=a-a avg",r"b'=b-b avg",r"c'=c-c avg",'Octant']
		for i in range(11):											# Writing header of file 
			myworkbook.cell(row=2,column=i+1).value=lst[i]	
		octant=[]
		a_avg=df['a'].mean()                                        # Finding average of u,v and w                 
		b_avg=df['b'].mean()
		c_avg=df['c'].mean()
		...
		myworkbook.cell(row=3,column=5).value=round(a_avg,3)
		myworkbook.cell(row=3,column=6).value=round(b_avg,3)
		myworkbook.cell(row=3,column=7).value=round(c_avg,3)

		for i in df.index:
			myworkbook.cell(row=i+3,column=1).value=df['T'][i]
			myworkbook.cell(row=i+3,column=2).value=df['a'][i]
			myworkbook.cell(row=i+3,column=3).value=df['b'][i]
			myworkbook.cell(row=i+3,column=4).value=df['c'][i]
			myworkbook.cell(row=i+3,column=8).value=round(df['a'][i]-a_avg,3)
			myworkbook.cell(row=i+3,column=9).value=round(df['b'][i]-b_avg,3)
			myworkbook.cell(row=i+3,column=10).value=round(df['c'][i]-c_avg,3)
			myworkbook.cell(row=i+3,column=11).value=find_octant(df['a'][i]-a_avg,df['b'][i]-b_avg,df['c'][i]-c_avg)
			octant.append(find_octant(df['a'][i]-a_avg,df['b'][i]-b_avg,df['c'][i]-c_avg))
